...
class DcsBiosExportParser extends EventEmitter {
  ...
  constructor(options = {}) {
    ...
    var result = jsonParser.parseFiles(files);
    this._aircraftData = result.aircraftData;
    this._controls = result.controls;
    this._addressLookup = result.addressLookup;

    // Create the UDP client.
    this._client = new UdpClient(this._options);

    // Create a buffer to hold all the incoming data from the DCS BIOS exporter. Although this buffer holds the data for
    // all controls, it's mainly used to re-construct strings when there's only a partial string update.
