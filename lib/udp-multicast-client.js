...
class UdpMulticastClient extends EventEmitter {
  ...
  start() {
    ...
    this.client.on('listening', () => {
      ...
      this.client.addMembership(this._config.multicastAddress);
    });
    ...
    this.client.bind(this._config.receivePort);
  }
  ...
  sendMessage(message) {
    ...
    return new Promise((resolve, reject) => {
      this.client.send(message, this._config.sendPort, '127.0.0.1', (error) => {
        if (error) {
