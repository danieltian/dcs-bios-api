...
class DcsBiosJsonParser {
   ...
   * @param {lookupObject} lookupObject - address lookup object
   */
  ...
  // Parse the JSON data and add the address ID to the address lookup object.
  parseJson(json, addressLookup) {
    ...
    var controls = getValues(json).reduce((array, category) => {
      Array.prototype.push.apply(array, getValues(category));
      return array;
    }, []);
...
export default DcsBiosJsonParser;
